...
function lineChart(config)
{
	...
	var formatDataValue =d3.time.format(config.x.tickformat);
		formatData = function(d){ return formatDataValue(d);};
	var toolTipFormatDataValue =d3.time.format(config.tooltip.format);
		toolTipFormatData = function(d){ return toolTipFormatDataValue(d);};
	...
	this.drawChart=function(data)
	{
		...
		yScale.domain([
			d3.min(linedata, function(c) { return d3.min(c.values, function(v) { return v.indexValue; }); }),
			d3.max(linedata, function(c) { return d3.max(c.values, function(v) { return v.indexValue; }); })
		]);
	...
	this.multiMousemove=function(mouse)
	{
	...
	this.mousemove=function(mouse) {
		...
		var infoHtml ="<div class='aass'>"+toolTipFormatData(xDate)+"</div>";
		infoHtml +="<ul>"
...
function areaChart(config)
{
	...
	this.drawChart=function(data)
	{
		...
		var browsers = stack(color.domain().map(function(name) {
			return {
				name: name,
				values: data.map(function(d) {
					return {key: d.key, y: d[name] * 1};
				})
			};
		}));

		// Find the value of the day with highest total value
		var maxKeyVal = d3.max(data, function(d){
		var vals = d3.keys(d).map(function(key){ return key !== "key" ? d[key] : 0 });
			return d3.sum(vals);
		});

		// Set domains for axes
		xScale.domain(d3.extent(data, function(d) { return d.key; }));
		yScale.domain([0, maxKeyVal])
		...
		////////////////////////////////////////////////////////////////////////
		/* 메인그래프 legend(범례) */
		if(config.legend.show==true) lineChartLegend(browsers,config);
	...
	this.mousemove=function(mouse) {
		...
		var chartDoc =document.getElementById(config.chartId);
		var lines = chartDoc.getElementsByClassName('area');
		//tooltip info box layout
		var xDate = xScale.invert(mouse[0]);
		var infoHtml ="<div>"+toolTipFormatData(xDate)+"</div>";
		infoHtml +="<ul>"
		...
		d3.selectAll("#"+config.chartId+" .tooltipPerLine")
			.attr("transform", function(d, i) {
					...
					tipData.push({name:d.name,val:yScale.invert(pos.y)});
					return "translate(" + mouse[0] + "," + pos.y +")";
			});
		var tipCnt = tipData.length-1;
		for(var i=tipCnt;i>=0;i--)
		{
			if(i==0) var preVal = 0;
			else var preVal = tipData[i-1].val;
		
			var tipVal = tipData[i].val - preVal;
			infoHtml+="<li style='color:"+color(tipData[i].name)+"'>"+tipData[i].name+" : <span>"+yFormatData(tipVal)+"</span></li>"
		}
		infoHtml +="</ul>";
		var posX=winMousePos[0]+20,posY=winMousePos[1];
			posY=height/2+50;
		var offsets = document.getElementById(config.chartId).getBoundingClientRect();
	...
	this.reDrawChart=function(data)
	{
		...
		/* 메인 그래프 */
		var maxKeyVal = d3.max(dataSet, function(d){
		var vals = d3.keys(d).map(function(key){ return key !== "key" ? d[key] : 0 });
			return d3.sum(vals);
		});
		...
		//그래프 갱신
		var focuslineGroups =  focus.selectAll(".browser").data(browsers).transition();
		focuslineGroups.select(".area").attr("d",  function(d){return area(d.values)})
