...
var initConfig = function(config)
{
	...
	if (typeof(config.x.tickformat)== 'undefined')config.x.tickformat="";
	/* y축 기본값 설정 */
	...
	if (typeof(config.tooltip.format) == 'undefined') config.tooltip.format=config.x.tickformat;
	/* 범례 */
	...
	/* pie 차트 : 도넛 */
	if (typeof(config.innerRadius) == 'undefined') config.innerRadius=0;
